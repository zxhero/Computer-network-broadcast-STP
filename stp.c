...
bool    compare_undesigned_port_pri(stp_port_t *p, stp_port_t *t){  //if p > t return true
    if(p->designated_root > t->designated_root){   return 0;	}
    else if(p->designated_root < t->designated_root)   return 1;
    else{
        if(p->designated_cost > t->designated_cost){    return 0;    }
        else if(p->designated_cost < t->designated_cost)    return 1;
        else{
            if(p->designated_switch > t->designated_switch){  return 0;    }
            else if(p->designated_switch < t->designated_switch)    return 1;
            else{
                if(p->designated_port > t->designated_port){  return 0;    }
                else return 1;
            }
        }
    }
...
bool    find_root_port(stp_t *stp, stp_port_t *p){
    if(!stp->root_port){
        stp->root_port = p;
    }
    else if(!compare_undesigned_port_pri(stp->root_port,p)){
        stp->root_port = p;
    }
    else    return 0;
    return 1;
		...
		struct stp_config *config)
{
	...
	if(p->designated_root > ntohll(config->root_id)){   pri = 1;	}
        ...
        if(p->designated_cost > ntohl(config->root_path_cost)){    pri = 1;    }
        else if(p->designated_cost < ntohl(config->root_path_cost));
        else{
            if(p->designated_switch > ntohll(config->switch_id)){  pri = 1;    }
